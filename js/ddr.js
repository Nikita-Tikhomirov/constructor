const ddr = {
  ...
  template: `
  <div ref='wrapper' :style="style" class="yoyoo-ddr" :class="getClassNames" @touchstart="handleMouseDown" @mousedown="handleMouseDown">
    <div v-if="resizable" class='resize-handler-wrapper'>
      <span v-for="item in resizeHandler" :data-resizetype="item" :key="item" :style="getNewHandler(item)" class="resize-handler" :class="item"/>
    </div>
    <span v-if="rotatable" :style="rotateHandler" data-type='rotate' class='rotate-handler' />
    <slot></slot>
  </div>
  `
}